...
func TestBucketConfigReadsReachHilt(t *testing.T) {
	...
	// A service key's worth of chains and actions: every permission in the
	// s3perm vocabulary (each asserted valid, so a renamed one fails here
	// instead of silently narrowing the set), so the operation is the only
	// reason left for the fast path to defer.
	allActions := []string{
	...
	for _, p := range allActions {
		require.True(t, s3perm.Valid(p), "%s is not in the s3perm vocabulary", p)
	}
	allChains := commandChains(t, spaceIssuer, accessKey, agent, s3perm.CommandsFor(allActions...))
