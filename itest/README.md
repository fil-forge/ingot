...
| `TestForgeEncryption` | FIL-495's encryption suite, on a default-config stack (256 MiB blobs, 256 KiB chunks): tampered-ciphertext rejection on whole and ranged GETs (corrupt the spooled envelope's final chunk — the failure is a mid-body stream error, headers are already out), DELETE of a multipart-created object releasing every part blob through piri, and the cheap 5 GiB+1 `EntityTooLarge` probe. The skipped subtests are FIL-495 assertions blocked on FIL-485/489/484 — each skip names its blocker. | ~3 min |
| `TestForgeMaxSizePart` | Exactly 5 GiB — the AWS-matching max part size — as one multipart part (21 internal blobs at the default `max_blob_size`): HEAD, ranged spot checks across chunk and blob boundaries, full stream-compared read-back. This test is why `bucket.DefaultMaxBlobSize` sits an envelope allowance under piri's 266338304-byte piece cap (the old 256 MiB default was unshippable). **Skipped unless `INGOT_ITEST_BIG=1`** (CI sets it): ~10–15 GiB of disk churn, several minutes. | minutes (gated) |
