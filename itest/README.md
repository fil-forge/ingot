...
| `TestForgeSpoolBudget` | The spool sweeper on a dedicated stack with a 4 MiB `spool_max_bytes` and both retention windows off (`testdata/config-spoolbudget.yaml`): 16 MiB of 1 MiB objects, spool usage falls to the budget within 2 minutes, and every object reads back byte-exact. | ~2 min |
