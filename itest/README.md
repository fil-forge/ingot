...
| `TestForgeAWSCLI` | A real, unmodified AWS CLI v2 (`amazon/aws-cli`, pinned in `forge_awscli_test.go`) round-trips a 20 MiB object with `aws s3 cp`. The CLI picks multipart itself at its 8 MiB switchover and sends its default request checksums (CRC64NVME, declared on CreateMultipartUpload and carried per part); the bytes come back exact, HEAD shows a 3-part ETag and the full-object checksum. The stack streams every CompleteMultipartUpload response (a 1ms `complete_keepalive_interval`, `testdata/config-keepalive.yaml`, dedicated stack), so the CLI parses the declaration-whitespace-result form. The CLI runs in its own container and reaches ingot's host-mapped port via `host.docker.internal`. | ~2 min |
