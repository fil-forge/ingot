...
func TestForgeS3Compat(t *testing.T) {
	...
	// Vectors tagged large move gigabytes (the 5 GiB copy-source limit needs
	// a source over 5 GiB); they run only on request.
	if os.Getenv("INGOT_S3COMPAT_LARGE") == "" {
		runOpts = append(runOpts, s3tests.Skip("large vectors run only with INGOT_S3COMPAT_LARGE=1", s3tests.Tags("large")))
	}
